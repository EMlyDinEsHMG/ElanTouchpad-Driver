...
ApplePS2Keyboard * ApplePS2Keyboard::probe(IOService * provider, SInt32 * score)
{
  ...
  if (!success) {
	...
	if (success)
	  IOLog("%s: probe succeeded after reset\n", getName());
	else
	  IOLog("%s: probe failed\n", getName());
  }
...
bool ApplePS2Keyboard::start(IOService * provider)
{
  ...
  //
  // Installing our MouseTouchpad Notification Handler
  //
    _device->installPS2NotificationAction(this, OSMemberFunctionCast(PS2NotificationAction, this, &ApplePS2Keyboard::receiveMouseTouchpadNotifications ));
    
  _mouseTouchpadNotificationHandlerInstalled = true;
